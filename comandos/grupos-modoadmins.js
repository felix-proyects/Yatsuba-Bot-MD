let handler = async (m, { conn, args, isAdmin, isBotAdmin, groupMetadata }) => {
  ...
  if (!m.isGroup) return m.reply('*❖ Este comando solo puede usarse en grupos.*', m, rcanal)
  ...
  if (!isAdmin) {
    return m.reply('*❖ Solo admins pueden usar este comando.*', m, rcanal)
  }
  ...
  if (/^(on|activar|true)$/i.test(args[0])) {
    if (chat.modoadmin) {
      return m.reply('*❖ Este modo ya estaba activado.*', m, rcanal)
    }
    chat.modoadmin = true
    return m.reply('*❖ Modo solo admins activado.*', m, rcanal)
  }
  ...
  if (/^(off|desactivar|false)$/i.test(args[0])) {
    ...
    return m.reply('*❖ Modo solo admins desactivado.*', m, rcanal)
  }
